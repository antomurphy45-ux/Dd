...
async function runtime() {
  if (!runtimePromise) runtimePromise = (async () => {
    ...
    // Phase 32.45: Pyodide 0.29.5+ resolves Node runtime assets from the
...
function json(status, data, extra={}) {
  return new Response(JSON.stringify(data), {
    status,
    headers: { "content-type": "application/json; charset=utf-8", "cache-control": "no-store", "x-construction-control-function": "32.45", ...extra },
  });
}
...
async function seedIfMissing(store) {
  ...
  const result = await store.set(DB_KEY, seed, { metadata: { format: "sqlite", version: "32.45" } });
  return result.etag;
}
...
async function putDatabase(store, bytes, etag) {
  const options = { metadata: { format: "sqlite", version: "32.45" }, onlyIfMatch: etag };
