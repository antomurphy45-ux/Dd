...
async function runtime() {
  if (!runtimePromise) runtimePromise = (async () => {
    ...
    // Phase 32.46: Node and Pyodide have separate filesystems.
    // The Node mkdir() calls above do NOT create directories inside
    // Pyodide's Emscripten filesystem. Create the virtual directories
    // before calling pyodide.FS.writeFile(), otherwise Pyodide raises
    // ErrnoError { errno: 44 } (ENOENT).
...
async function seedIfMissing(store) {
  ...
  const result = await store.set(DB_KEY, seed, { metadata: { format: "sqlite", version: "32.46" } });
  return result.etag;
}
...
async function putDatabase(store, bytes, etag) {
  const options = { metadata: { format: "sqlite", version: "32.46" }, onlyIfMatch: etag };
