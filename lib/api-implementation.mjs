...
function json(status, data, extra={}) {
  return new Response(JSON.stringify(data), {
    status,
    headers: { "content-type": "application/json; charset=utf-8", "cache-control": "no-store", "x-construction-control-function": "32.40", ...extra },
  });
}
...
async function seedIfMissing(store) {
  ...
  const result = await store.set(DB_KEY, seed, { metadata: { format: "sqlite", version: "32.40" } });
  return result.etag;
}
...
async function putDatabase(store, bytes, etag) {
  const options = { metadata: { format: "sqlite", version: "32.40" }, onlyIfMatch: etag };
