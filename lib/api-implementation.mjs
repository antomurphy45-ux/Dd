...
// Netlify normally injects Blobs credentials automatically. This project has
// repeatedly reached production without that injection, so use the standard
// runtime variables explicitly as a robust fallback. Never hard-code secrets.
function blobStore() {
  ...
  if (!siteID || !token) {
    ...
    throw new Error(`Netlify Blobs credentials are unavailable in the Function runtime (missing: ${missing}). Set the missing variable(s) in Netlify with Functions scope, then redeploy.`);
  }
...
function json(status, data, extra={}) {
  return new Response(JSON.stringify(data), {
    status,
    headers: { "content-type": "application/json; charset=utf-8", "cache-control": "no-store", "x-construction-control-function": "32.42", ...extra },
  });
}
...
async function seedIfMissing(store) {
  ...
  const result = await store.set(DB_KEY, seed, { metadata: { format: "sqlite", version: "32.42" } });
  return result.etag;
}
...
async function putDatabase(store, bytes, etag) {
  const options = { metadata: { format: "sqlite", version: "32.42" }, onlyIfMatch: etag };
