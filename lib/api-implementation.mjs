...
async function runtime() {
  if (!runtimePromise) runtimePromise = (async () => {
    ...
    // Phase 32.48: Netlify's deployed /var/task filesystem is read-only.
...
function json(status, data, extra={}) {
  return new Response(JSON.stringify(data), {
    ...
    headers: {
      ...
      "x-construction-control-function": "32.48",
      ...extra
    },
  });
}
...
async function seedIfMissing(store) {
  ...
  const result = await store.set(DB_KEY, seed, { metadata: { format: "sqlite", version: "32.48" } });
  return result.etag;
}
...
async function putDatabase(store, bytes, etag) {
  const options = { metadata: { format: "sqlite", version: "32.48" }, onlyIfMatch: etag };
