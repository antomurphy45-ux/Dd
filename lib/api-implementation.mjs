...
// Do not derive ROOT from module metadata here. Netlify bundles ESM into the
// CommonJS Lambda runtime, where the module URL is not guaranteed to exist. The Lambda
// task root is the directory containing the files listed in included_files.
...
// Netlify's current documentation says siteID, deployID and token are supplied
// automatically when @netlify/blobs is used from a Function. Try that zero-config
// path first. If the platform context is unavailable, use explicit credentials only
// when deliberately supplied by the project. No secret is stored in the repository.
function blobStore() {
...
async function runtime() {
  if (!runtimePromise) runtimePromise = (async () => {
    ...
    // Phase 32.44: Pyodide 0.29.5+ resolves Node runtime assets from the
    // installed npm package. Do not pass a browser CDN indexURL here.
...
function json(status, data, extra={}) {
  return new Response(JSON.stringify(data), {
    status,
    headers: { "content-type": "application/json; charset=utf-8", "cache-control": "no-store", "x-construction-control-function": "32.44", ...extra },
  });
}
...
async function seedIfMissing(store) {
  ...
  const result = await store.set(DB_KEY, seed, { metadata: { format: "sqlite", version: "32.44" } });
  return result.etag;
}
...
async function putDatabase(store, bytes, etag) {
  const options = { metadata: { format: "sqlite", version: "32.44" }, onlyIfMatch: etag };
