...
function json(status, data, extra={}) {
  return new Response(JSON.stringify(data), { status, headers: { "content-type":"application/json; charset=utf-8", "cache-control":"no-store", "x-construction-control-function":"32.49", ...extra } });
}
...
async function putDatabase(store, bytes, etag) {
  return store.set(DB_KEY, bytes, { metadata:{format:"sqlite",version:"32.49"}, onlyIfMatch:etag });
}

// Phase 32.49: the persistent Blobs database is restored after app.py is imported.
...
# Phase 32.49: repair missing demo accounts in an existing persistent database.
...
async function handleRequest(req) {
  try {
    ...
    if (pathname === "/healthz") return json(200,{status:"ok",platform:"netlify",storage:"netlify-blobs",phase:"32.49"});
