...
function blobStore() {
  ...
  try {
    return getStore(STORE_NAME, { region: REGION });
  } catch (automaticError) {
    if (siteID && token) {
      return getStore(STORE_NAME, { region: REGION, siteID, token });
    }
    const missing = [
      !siteID ? "SITE_ID" : null,
    ...
    throw new Error(
      `Netlify Blobs automatic Function context is unavailable (missing explicit fallback: ${missing || "none"}). ${automaticError?.message || automaticError}`
    );
  }
}
...
async function runtime() {
  if (!runtimePromise) runtimePromise = (async () => {
    ...
    // Phase 32.49: Netlify's deployed /var/task filesystem is read-only.
    // Pyodide must cache downloaded packages in writable /tmp storage.
    ...
    pyodide.FS.writeFile(
      `${WORK}/render_seed.db`,
      await readFile(path.join(ROOT, "netlify_seed.db"))
    );

    // sqlite3 is an unvendored Pyodide package.
    // Load it before importing app.py, which imports sqlite3 at module load time.
...
function json(status, data, extra={}) {
  return new Response(JSON.stringify(data), {
    ...
    headers: {
      ...
      "x-construction-control-function": "32.49",
      ...extra
    },
  });
}
...
async function seedIfMissing(store) {
  ...
  const result = await store.set(DB_KEY, seed, { metadata: { format: "sqlite", version: "32.49" } });
  return result.etag;
}
...
async function putDatabase(store, bytes, etag) {
  const options = { metadata: { format: "sqlite", version: "32.49" }, onlyIfMatch: etag };
...
async function handleRequest(req) {
  try {
    ...
    if (pathname === "/healthz") return json(200, {
      status: "ok",
      platform: "netlify",
    ...
    while (attempts++ < 3) {
      ...
      // Phase 32.49: repair the persistent demo login after restoring the
      // Netlify Blobs database. Older persistent databases can contain
      // companies/data but be missing the packaged demo accounts. Do not
      // overwrite an existing demo account; only create it when absent.
      await py.runPythonAsync(`
import app, secrets
_cc = app.db()
try:
    _company = _cc.execute("SELECT id FROM companies WHERE id=?", ("C1",)).fetchone()
    if _company:
        _owner = _cc.execute("SELECT id FROM users WHERE lower(email)=lower(?)", ("owner@demo.local",)).fetchone()
        if not _owner:
            _cc.execute("INSERT INTO users(id,company_id,name,email,password_hash,active,role) VALUES(?,?,?,?,?,?,?)", (secrets.token_hex(8), "C1", "Company Owner", "owner@demo.local", app.pw_hash("DemoPass!123"), 1, "Company Administrator"))
        _manager = _cc.execute("SELECT id FROM users WHERE lower(email)=lower(?)", ("manager@demo.local",)).fetchone()
        if not _manager:
            _cc.execute("INSERT INTO users(id,company_id,name,email,password_hash,active,role) VALUES(?,?,?,?,?,?,?)", (secrets.token_hex(8), "C1", "Construction Manager", "manager@demo.local", app.pw_hash("DemoPass!123"), 1, "Construction Manager"))
        _cc.commit()
finally:
    _cc.close()
`);
