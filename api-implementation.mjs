...
function json(status, data, extra={}) {
  return new Response(JSON.stringify(data), { status, headers: { "content-type":"application/json; charset=utf-8", "cache-control":"no-store", "x-construction-control-function":"32.50", ...extra } });
}
...
async function seedIfMissing(store) {
  ...
  const result = await store.set(DB_KEY, seed, { metadata:{format:"sqlite",version:"32.50"} });
  return result.etag;
}
...
async function putDatabase(store, bytes, etag) {
  return store.set(DB_KEY, bytes, { metadata:{format:"sqlite",version:"32.50"}, onlyIfMatch:etag });
}

// Phase 32.50: the persistent Blobs database is restored after app.py is imported.
...
# Phase 32.50: repair missing demo accounts in an existing persistent database.
...
try:
    ...
    for uid, company_id, name, email, role in demo_users:
        ...
        if existing:
            # Phase 32.50: these are the built-in demo accounts. If an existing
...
async function handleRequest(req) {
  try {
    ...
    if (pathname === "/healthz") return json(200,{status:"ok",platform:"netlify",storage:"netlify-blobs",phase:"32.50"});
